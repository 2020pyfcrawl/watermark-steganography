...
public class F4 {
	public static void main(String[] args) throws IOException {
		add_F4("test.txt","more_data_48.jpg");
//		decode_F4("test1.jpg");
//		System.out.println(-3 % 2 == -1);
		decode_F4_FromFile("code_F4.txt");
	}
	...
	public static void add_F4(String txtfile,String imagePath) throws IOException {
		
		FileWriter writer = new FileWriter("code_F4.txt",false);
		...
		//DCT co is get now
		FileWriter writer_ori = new FileWriter("ori_F4.txt",false);
		...
		for(int i = 0; i < bits_length; ) {
			...
			if(ret) {
				i++;
				//System.out.println("this bit is " + bit + " this value is now " + int_zg_dct[block_id][index]);
			}
			else {
				//do nothing
			}
		...
		FileWriter writer_new = new FileWriter("new_F4.txt",false);
		for(int i = 0; i < blocknums; i++) {
			for(int j = 0; j < elenums; j++) {
		...
		JPEG_DCT_Zigzag.transformGray(imagePath,"testF4.jpg","jpg",new_data);
		
		
	}
	
	public static void decode_F4(String imagePath) throws IOException{
		double zigzag_dct[][] =JPEG_DCT_Zigzag.getSequence(imagePath);
		int [][] int_zg_dct = doubleToint(zigzag_dct);		
		int blocknums = int_zg_dct.length;
		int elenums = int_zg_dct[0].length;
	...
	public static void decode_F4_FromFile(String decodefile) throws IOException{
		...
		String message = bitsstream.bitsstreamToString(decode);
		System.out.println(message);
		FileWriter writer = new FileWriter("decode_F4.txt",false);
